...
+   Implement a very low-level C-like language with sensible syntax that contains very basic pointers. The language will only contain int type and the plus operator for pointer arithmetic.
+   You can print either the index/address of a pointer or the value at the address the pointer is pointer at by using '*' as a dereference tool.
