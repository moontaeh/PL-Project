...
export function evaluate(varEnv, pointEnv, e) {
    switch (e.tag) {
        case 'int':
            return e;
        case 'plus': {
        ...
        case 'var': {
            ...
            if (name.indexOf("*") === 0) {
                ...
                if (varEnv.has(name)) {
                    return pointEnv.get(varEnv.get(name).index);
                }
                else {
...
export function execute(varEnv, pointEnv, prog) {
    const output = [];
    for (const s of prog) {
        switch (s.tag) {
            case 'define': {
                ...
                if (s.type === L.tyint) {
                    ...
                    if (v.tag === "int") {
                        ...
                        varEnv.set(s.id, L.pointer(ptr.index, s.id));
                    }
                }
                break;
            }
                ...
                if (s.loc.tag === 'var') {
                    if (varEnv.has(s.loc.name)) {
                        if (varEnv.get(s.loc.name).tag === 'int') {
                            ...
                            if (rhs.tag === 'int') {
                                pointEnv.update(varEnv.get(s.loc.name).index, rhs);
                            }
